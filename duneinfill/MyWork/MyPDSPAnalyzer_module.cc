...
*/
  ...
  std::vector<std::map<int,unsigned int>> GetNeighboursForRadii(art::Event const &evt,
    const std::vector<float>& rangeCuts, const std::vector<float>& chargeRangeCuts, const std::vector<const recob::SpacePoint*> &sps, const std::string spacePointLabel) {
    ...
    std::map<unsigned int,float> chargeMap = GetSpacePointChargeMap(evt, spacePointLabel);
  ...
  TFile * OpenFile(const std::string filename) {
    ...
    if (cet::file_exists(filename)) {
      ...
      if (!theFile ||theFile->IsZombie() || !theFile->IsOpen()) {
        delete theFile;
        theFile = 0x0;
        throw cet::exception("MyPDSPAnalyzer_module.cc") << "Could not open " << filename;
      }
    }
    else {
      ...
      if (!found) {
        throw cet::exception("MyPDSPAnalyzer_module.cc") << "Could not find " << filename;
      }
      ...
      if (!theFile ||theFile->IsZombie() || !theFile->IsOpen()) {
        delete theFile;
        theFile = 0x0;
        throw cet::exception("MyPDSPAnalyzer_module.cc") << "Could not open " << found_filename;
      }
    }
    return theFile;
  }
...
class pduneana::MyPDSPAnalyzer : public art::EDAnalyzer {
public:
  explicit MyPDSPAnalyzer(fhicl::ParameterSet const& p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  MyPDSPAnalyzer(MyPDSPAnalyzer const&) = delete;
  MyPDSPAnalyzer(MyPDSPAnalyzer&&) = delete;
  MyPDSPAnalyzer& operator=(MyPDSPAnalyzer const&) = delete;
  MyPDSPAnalyzer& operator=(MyPDSPAnalyzer&&) = delete;
...
private:
  ...
  protoana::MyProtoDUNETrackUtils myTrackUtil;
  ...
  // My extra labels
  std::string fPandoraCaloTag;
  std::string fShowerCaloTag;
  std::string fPandora2TrackTag;
  std::string fPandora2ShowerTag;
  std::string fEmtrkmichelidTag;
...
pduneana::MyPDSPAnalyzer::MyPDSPAnalyzer(fhicl::ParameterSet const& p)
  : EDAnalyzer{p}  ,
  ...
  // My extra labels
  fPandoraCaloTag    (p.get<std::string> ("PandoraCaloTag")),
  fShowerCaloTag     (p.get<std::string> ("ShowerCaloTag")),
  fPandora2TrackTag  (p.get<std::string> ("Pandora2TrackTag")),
  fPandora2ShowerTag (p.get<std::string> ("Pandora2ShowerTag")),
  fEmtrkmichelidTag  (p.get<std::string> ("EmtrkmichelidTag")),
...
void pduneana::MyPDSPAnalyzer::analyze(art::Event const & evt) {
    ...
    for (const recob::PFParticle & pfp : (*pfpVec)) {
      ...
      if (tempTrack) {
        ...
        std::pair<double, int> vertex_michel_score =
            myTrackUtil.GetVertexMichelScore(*tempTrack, evt, fTrackerTag,
                                           fHitTag, fEmtrkmichelidTag);
        std::pair<double, double> vertex_michel_score_weight_by_charge =
            myTrackUtil.GetVertexMichelScore_weight_by_charge(*tempTrack, evt, fTrackerTag,
                                           fHitTag, fEmtrkmichelidTag);
  ...
  if( !evt.isRealData() ){
    ...
    if( !true_beam_particle ){
      MF_LOG_WARNING("MyPDSPAnalyzer") << "No true beam particle" << std::endl;
      return;
    }
    if (fVerbose) {
  ...
  if (!fSkipMVA) { 
    hitResults = new anab::MVAReader<recob::Hit, 4>(evt, fEmtrkmichelidTag );
  }
...
void pduneana::MyPDSPAnalyzer::beginJob() {
...
void pduneana::MyPDSPAnalyzer::endJob()
{
  dEdX_template_file->Close();
}

double pduneana::MyPDSPAnalyzer::lateralDist(TVector3 &n, TVector3 &x0, TVector3 &p){
  TVector3 x = ( (p - x0)*n )*n;
  return (x - (p - x0)).Mag();
}

void pduneana::MyPDSPAnalyzer::reset()
{
...
void pduneana::MyPDSPAnalyzer::BeamPFPInfo(
...
void pduneana::MyPDSPAnalyzer::BeamTrackInfo(
  ...
  if (!fSkipMVA) {
    std::pair<double, int> vertex_michel_score =
        myTrackUtil.GetVertexMichelScore(*thisTrack, evt, fTrackerTag, fHitTag, fEmtrkmichelidTag/*,
                                       0., -500., 500., 0., 500., 0.*/);
    reco_beam_vertex_nHits = vertex_michel_score.second;
    reco_beam_vertex_michel_score = vertex_michel_score.first;
    
    std::pair<double, double> vertex_michel_score_weight_by_charge =
        myTrackUtil.GetVertexMichelScore_weight_by_charge(*thisTrack, evt, fTrackerTag, fHitTag, fEmtrkmichelidTag);
  ...
  if (found_calo) {
    ...
    //For Prod3 only
    if (fCalorimetryTagSCE.substr(0, fCalorimetryTagSCE.find(":")) == "pandoracali") {
      auto pandoracalo = trackUtil.GetRecoTrackCalorimetry(*thisTrack, evt, fTrackerTag, fPandoraCaloTag);
      size_t this_index = 0;
...
void pduneana::MyPDSPAnalyzer::BeamShowerInfo(const art::Event & evt, const recob::Shower* thisShower) {
  reco_beam_type = 11;
  ...
  auto allHits = evt.getValidHandle<std::vector<recob::Hit> >(fHitTag);
  auto calo = showerUtil.GetRecoShowerCalorimetry(*thisShower, evt, fShowerTag, fShowerCaloTag);
  bool found_calo = false;
  size_t index = 0;
  for ( index = 0; index < calo.size(); ++index) {
    if (calo[index].PlaneID().Plane == 2) {
...
void pduneana::MyPDSPAnalyzer::TrueBeamInfo(
  ...
  if( fTrueToReco ){
    for( size_t i = 0; i < trueToPFPs[ true_beam_ID ].size(); ++i ){
      ...
      try{
        pandora2Track = pfpUtil.GetPFParticleTrack( *thePFP, evt, fPFParticleTag, fPandora2TrackTag );
      }
      catch( const cet::exception &e ){
        MF_LOG_WARNING("MyPDSPAnalyzer") << "pandora2Track object not found, moving on" << std::endl;
      }
      ...
      for( int j = 0; j < part->NumberDaughters(); ++j ){
        ...
        if( fTrueToReco ){
          for( size_t k = 0; k < trueToPFPs[ pi0_decay_part->TrackId() ].size(); ++k ){
            ...
            try{
              pandora2Shower = pfpUtil.GetPFParticleShower( *thePFP, evt, fPFParticleTag, fPandora2ShowerTag );
            }
            catch( const cet::exception &e ){
              MF_LOG_WARNING("MyPDSPAnalyzer") << "pandora2Shower object not found, moving on" << std::endl;
            }
...
void pduneana::MyPDSPAnalyzer::BeamInstInfo(const art::Event & evt) {
  ...
  if (evt.isRealData()) {
    ...
    try {
      ...
      MF_LOG_WARNING("MyPDSPAnalyzer") << "BeamEvent generator object not " <<
  ...
  if (evt.isRealData() && !fBeamlineUtils.IsGoodBeamlineTrigger(evt)) {
    ...
    MF_LOG_WARNING("MyPDSPAnalyzer") << "Failed beam quality check" << std::endl;
...
void pduneana::MyPDSPAnalyzer::DaughterPFPInfo(
    const art::Event & evt, const recob::PFParticle* particle,
    detinfo::DetectorClocksData const& clockData,
    anab::MVAReader<recob::Hit,4> * hitResults) {
  ...
  for (size_t daughterID : particle->Daughters()) {
    ...
    if (fSaveHits) {
      ...
      neighbourMap.insert(std::make_pair(0,GetNeighboursForRadii(evt,fNeighbourRadii, fChargeRadii, space_pts, fSpacePointLabel))); 
      ...
      // Get charge map 
      std::map<unsigned int,float> chargeMap = GetSpacePointChargeMap(evt, fSpacePointLabel);
    ...
    const recob::Track * pandoraTrack = pfpUtil.GetPFParticleTrack(
        *daughterPFP, evt, fPFParticleTag, fTrackerTag);
    const recob::Shower * pandoraShower = pfpUtil.GetPFParticleShower(
        *daughterPFP, evt, fPFParticleTag, fShowerTag);
    if (pandoraTrack != 0x0) {
    ...
    try{
      const recob::Track* pandora2Track = pfpUtil.GetPFParticleTrack( *daughterPFP, evt, fPFParticleTag, fPandora2TrackTag );
      ...
      if( pandora2Track ){
        ...
        auto dummy_caloSCE =
            trackUtil.GetRecoTrackCalorimetry(
                *pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE);
        bool found_calo = false;
        size_t index = 0;
          ...
          if (fRecalibrate) {
            std::vector<float> cali_dEdX_SCE
                = calibration_SCE.GetCalibratedCalorimetry(
                    *pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE, 2);
            ...
            std::vector<double> new_dQdX = calibration_SCE.CalibratedQdX(
                *pandora2Track, evt, fPandora2TrackTag,
                fPandora2CaloSCE, 2, -10.);
            for (auto dqdx : new_dQdX) {
              reco_daughter_allTrack_calibrated_dQdX_SCE.back().push_back(dqdx);
            }
            std::vector<double> efield = calibration_SCE.GetEFieldVector(
                *pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE, 2, -10.);
            for (auto ef : efield) {
        ...
        if (found_plane0) {
          ...
          std::vector<float> dEdX_plane0
              = (fRecalibrate ?
                 calibration_SCE.GetCalibratedCalorimetry(
                    *pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE, 0) :
        ...
        if (found_plane1) {
          ...
          std::vector<float> dEdX_plane1
              = (fRecalibrate ? 
                 calibration_SCE.GetCalibratedCalorimetry(
                    *pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE, 1) :
        ...
        if (!fSkipMVA) {
          std::pair<double, int> vertex_results =
              myTrackUtil.GetVertexMichelScore(
                  *pandora2Track, evt, fPandora2TrackTag, fHitTag, fEmtrkmichelidTag,
    ...
    try{
      const recob::Shower* pandora2Shower = pfpUtil.GetPFParticleShower( *daughterPFP, evt, fPFParticleTag, fPandora2ShowerTag );
      ...
      if( pandora2Shower ){
        ...
        const std::vector<art::Ptr<recob::Hit>> hits =
            showerUtil.GetRecoShowerArtHits(
                *pandora2Shower, evt, fPandora2ShowerTag);
        ...
        if (fGetCalibratedShowerEnergy) {
          ...
          auto calo = showerUtil.GetRecoShowerCalorimetry(
              *pandora2Shower, evt, fPandora2ShowerTag, fPandora2CaloSCE);
          bool found_calo = false;
          size_t index = 0;
          for (index = 0; index < calo.size(); ++index) {
            if (calo[index].PlaneID().Plane == 2) {
...
void pduneana::MyPDSPAnalyzer::BeamMatchInfo(
    const art::Event & evt, const recob::PFParticle * particle,
    const simb::MCParticle * true_beam_particle,
    detinfo::DetectorClocksData const& clockData) {
...
void pduneana::MyPDSPAnalyzer::DaughterMatchInfo(
...
void pduneana::MyPDSPAnalyzer::BeamForcedTrackInfo(
    const art::Event & evt, const recob::PFParticle * particle) {
  try{
    const recob::Track* pandora2Track = pfpUtil.GetPFParticleTrack( *particle, evt, fPFParticleTag, fPandora2TrackTag );
    if (fVerbose) std::cout << "pandora2 track: " << pandora2Track << std::endl;

    if( pandora2Track ){
      // michle score
      if (!fSkipMVA) {
        std::pair<double, int> vertex_michel_score = myTrackUtil.GetVertexMichelScore(*pandora2Track, evt, fPandora2TrackTag, fHitTag, fEmtrkmichelidTag);
        ...
        std::pair<double, double> vertex_michel_score_weight_by_charge =
            myTrackUtil.GetVertexMichelScore_weight_by_charge(*pandora2Track, evt, fPandora2TrackTag, fHitTag, fEmtrkmichelidTag);
      ...
      auto allHits = evt.getValidHandle<std::vector<recob::Hit> >(fHitTag);
      auto calo = trackUtil.GetRecoTrackCalorimetry(*pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE);
      size_t index = 0;
      ...
      if (found_plane) {
        ...
        //New Calibration
        if (fRecalibrate){
          std::vector< float > new_dEdX = calibration_SCE.GetCalibratedCalorimetry( *pandora2Track, evt, fPandora2TrackTag, fPandora2CaloSCE, 2);
    ...
    MF_LOG_WARNING("MyPDSPAnalyzer") << "beam pandora2Track object not found, moving on" << std::endl;
  }
}

void pduneana::MyPDSPAnalyzer::G4RWGridWeights(
    std::vector<std::vector<G4ReweightTraj *>> & hierarchy,
    std::vector<fhicl::ParameterSet> & pars,
    std::vector<std::vector<double>> & weights,
    G4MultiReweighter * multi_rw) {
...
void pduneana::MyPDSPAnalyzer::GetG4RWCoeffs(std::vector<double> & weights, std::vector<double> & coeffs) {
...
DEFINE_ART_MODULE(pduneana::MyPDSPAnalyzer)
